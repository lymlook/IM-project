    ...
    <el-dialog title="新增" :visible.sync="addFormVisible" width="35%">
      <el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
        ...
        <el-form-item label="菜单URL" prop="url">
          <el-input v-model="addForm.url" placeholder="/xxxxx" auto-complete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    loadData() {
      var menusUrl = this.Api + "Menus/MenusList"; // 获取菜单列表

      this.axios
        .get(menusUrl)
        .then(response => {
          this.tableData = response.data.menuList;
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            // 发保存请求
            var url = this.Api + "Menus/menusEdit";
            this.$axios
    ...
    handleDelete(index, row) {
      this.$confirm("确定删除吗？", "提示", {}).then(() => {
        ...
        var url = this.Api + "Menus/deleteMenus"; // 删除权限节点
        this.$axios
      ...
      this.$refs.addForm.validate(valid => {
        ...
        if (valid) {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            // 发保存请求
            var url = this.Api + "Menus/menusAdd"; // 添加权限
            this.$axios
              .post(url, this.$qs.stringify(params))
              .then(response => {
